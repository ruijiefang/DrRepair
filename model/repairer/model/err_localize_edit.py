...
class ErrLocalizeEditModel(Model):
    ...
    def forward_encode(self, batch):
        ...
        if self.c_model.type == "code_compiler":
            ...
            code_embeds = torch.cat([code_embeds1, code_embeds2], dim=2) #(batch, num_lines, total_lstm_layers * 2 * lstm_dim)
            # code_embeds_c = torch.cat([code_embeds_c1, code_embeds_c2], dim=2) #(batch, num_lines, total_lstm_layers * 2 * lstm_dim)
            text_embeds2, text_embeds_output = self.embed_stuff_for_transformer(text_indices, text_wembs, self.text_seq_embedder2, self.text_linear_after_2nd_lstm, self.text_hidden_after_2nd_lstm)
...
#                print("_msg_embeds_output: ", _msg_embeds_output.shape)
    ...
    def embed_stuff_for_transformer(self, inp_indices, inp_wembs, seq_embedder, linear_layer, hidden_linear_layer):
        """
        Embed the sequences.
        """
        ...
        lstm_output_fullypadded[:seq_output.size(0)] = seq_output #(true_slen, `batch`, 2*lstm_dim)
